...
ALTER TABLE `staff` ADD CONSTRAINT `staff_role_id_roles_id_fk` FOREIGN KEY (`role_id`) REFERENCES `roles`(`id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE `loan_details` ADD CONSTRAINT `loan_details_group_id_projects_id_fk` FOREIGN KEY (`group_id`) REFERENCES `projects`(`id`) ON DELETE no action ON UPDATE no action;